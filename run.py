...
# Function to get audio features with retry mechanism
...
track_id = '5rEAzUw92fhaeZiOc20jLc'
try:
    features = get_audio_features_with_retry(track_id)
    print(features)
except Exception as e:
    print("Error:", e)
###
...
cluster = data1.groupby(by=data1['kmeans'])
data2.pop('kmeans')
df1 = cluster.apply(lambda x: x.sort_values(["popularity"], ascending=False))
if 'kmeans' in df1.columns:
    df1.drop(columns=['kmeans'], inplace=True)
